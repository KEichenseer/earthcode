...
ylabpush = -0.14009*(all_period_start[firstindex]-all_period_end[lastindex]) + ylabpush_custom
...
plot(0,0, xlim = c(min(periodbase), max(periodtop)), ylim = ylimit, xaxs="i",
...
text(-ylabpush-all_period_start[lastindex], mean(ylimit),
     labels = ylable, srt = 90, adj = c(0.5,0.5), xpd = TRUE, cex=lablesize)


axis_at <- seq(0, -all_period_start[lastindex], by = -age_at)
...
text(mean(c(min(periodbase), max(periodtop))),y1 -xlabfactor*(y1-y2),labels = xlable, xpd = TRUE)
